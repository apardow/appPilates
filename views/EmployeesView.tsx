...
}> = ({ employee, onBack, onSave }) => {
  const [formData, setFormData] = useState<
    Omit<IEmployee, 'id'> & { id?: number }
  >(
    employee || {
      name: '',
      lastName1: '',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4 text-gray-700">Imagen</h3>
            <div className="w-full h-40 border-2 border-dashed rounded-lg flex flex-col justify-center items-center text-gray-400 bg-gray-50 hover:bg-gray-100 cursor-pointer">
              <ImageIcon size={48} />
              <p className="text-sm mt-2">Agregar foto referencial</p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4 text-gray-700">Estado</h3>
            <select
              name="status"
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4 text-gray-700">Datos personales</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                onChange={handleChange}
                placeholder="Nombre"
                className="p-2 border rounded-md focus:ring-purple-500 focus:border-purple-500"
              />
              <input
                name="lastName1"
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4 text-gray-700">Datos empleado</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                name="company"
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4 text-gray-700">Sucursales</h3>
            <div className="flex flex-wrap gap-2">
              {initialBranches.map((branch) => (
                <button
                  ...
                  className={`py-1 px-3 rounded-full text-sm font-semibold transition-colors ${formData.branches.includes(branch.name) ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                >
                  {branch.name}
                </button>
              ))}
          ...
          <tbody className="divide-y divide-gray-200">
            {initialEmployees.map((emp) => (
              <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="p-4">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${emp.status === 'Habilitado' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                  >
                    {emp.status}
                  </span>
                </td>
