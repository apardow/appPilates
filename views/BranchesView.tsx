...
const BranchForm: React.FC<{ branch: IBranch | null; onBack: () => void; onSave: (data: IBranch) => void }> = ({ branch, onBack, onSave }) => {
    const [formData, setFormData] = useState<Omit<IBranch, 'id' | 'code'> & { id?: number, code?: string }>(
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSave({ ...formData, id: formData.id || Date.now(), code: formData.code || 'AUTOGEN' } as IBranch);
    };

    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-6 border-b pb-4">
                <h2 className="text-2xl font-bold text-gray-800">{branch ? 'Editar Sucursal' : 'Crear Sucursal'}</h2>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input name="name" value={formData.name} onChange={handleChange} placeholder="Nombre de la sucursal" className="w-full p-2 border rounded-md focus:ring-purple-500 focus:border-purple-500" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Código</label>
                    <input name="code" value={formData.code || ''} placeholder={branch ? '' : "Se generará automáticamente"} className="w-full p-2 border rounded-md bg-gray-100 text-gray-500 cursor-not-allowed" disabled />
                </div>
                <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Dirección</label>
                    <input name="address" value={formData.address} onChange={handleChange} placeholder="Dirección completa" className="w-full p-2 border rounded-md focus:ring-purple-500 focus:border-purple-500" />
                </div>
                <div>
...
const BranchesView: React.FC = () => {
    ...
    const handleSelect = (branch: IBranch) => { setSelectedBranch(branch); setView('form'); };
    const handleCreateNew = () => { setSelectedBranch(null); setView('form'); };
    const handleBackToList = () => { setView('list'); setSelectedBranch(null); };
    ...
    if (view === 'form') {
        return <BranchForm branch={selectedBranch} onBack={handleBackToList} onSave={handleSave} />;
    }
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleCreateNew} className="bg-purple-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-purple-700 flex items-center transition-colors">
                    <Plus size={20} className="mr-2" />Crear Sucursal
                </button>
            </div>
