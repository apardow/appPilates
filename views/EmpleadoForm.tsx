...
const EmpleadoForm: React.FC = () => {
  ...
  const [archivoNuevoCertificado, setArchivoNuevoCertificado] =
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || 'Ocurrió un error al guardar el empleado.',
        );
      }
      navigate('/empleados');
    } catch (err: any) {
  ...
  const handleCertificadoSubmit = async (
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/empleados/${id}/certificados`,
        {
          method: 'POST',
  ...
  const handleCertificadoDelete = async (certificadoId: number) => {
    if (
      !window.confirm('¿Estás seguro de que quieres eliminar este certificado?')
    )
      return;
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/certificados/${certificadoId}`,
        {
          method: 'DELETE',
          headers: { Accept: 'application/json' },
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || 'Error al eliminar el certificado.',
        );
      }
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <input
                  ...
                  value={empleado.direccion_completa}
                  onChange={handleInputChange}
                  className={inputStyle}
                />
              </div>
        ...
        <div className="mt-8 flex justify-end gap-4">
          ...
          <button
            type="submit"
            disabled={guardando}
            className="bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 disabled:bg-purple-300"
          >
      ...
      <div className="bg-white p-8 rounded-xl shadow-md">
        ...
        {!isEditing ? (
          <div className="bg-blue-50 border-l-4 border-blue-400 text-blue-700 p-4 rounded-md flex items-center">
            ...
            <p>
              Primero debe guardar el empleado para poder añadirle certificados.
            </p>
          </div>
        ) : (
          <>
              ...
              {certificados.length > 0 ? (
                <ul className="space-y-2">
                  {certificados.map((cert) => (
                    ...
                    >
                      <a
                        href={`https://api.espaciopilatescl.cl${cert.url_documento}`}
                        target="_blank"
                      ...
                      <button
                        onClick={() => handleCertificadoDelete(cert.id)}
                ...
                <p className="text-gray-500 italic">
                  No hay certificados cargados para este empleado.
                </p>
              )}
            </div>
            ...
            <form onSubmit={handleCertificadoSubmit}>
              ...
              <div className="flex justify-end mt-4">
                <button
                  type="submit"
                  disabled={subiendo}
                  className="flex items-center justify-center gap-2 bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 disabled:bg-blue-300"
                >
                  <RiUploadCloud2Line />
        ...
        {notificacion && (
          <div className="mt-4 text-green-600">{notificacion}</div>
        )}
      </div>
    </div>
  );
