...
const RatingsView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [calificacionesRes, sucursalesRes, empleadosRes] =
          await Promise.all([
  ...
  const calificacionesFiltradas = calificaciones.filter((cal) => {
    const pasaFiltroSurcursal =
      !filtros.sucursal || cal.sucursal.nombre === filtros.sucursal;
    const pasaFiltroServicio =
      !filtros.servicio || cal.servicio.nombre === filtros.servicio;
    const pasaFiltroMonitor =
      !filtros.monitor ||
      `${cal.empleado.nombre} ${cal.empleado.apellido_paterno}` ===
        filtros.monitor;
  ...
  if (cargando)
    return <div className="p-8 text-center">Cargando calificaciones...</div>;
  if (error)
    return (
      <div className="p-8">
        <div
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="bg-white p-4 rounded-xl shadow-md mb-6 flex flex-wrap gap-4 items-center">
        ...
        >
          ...
          {empleados.map((e) => (
            <option key={e.id} value={`${e.nombre} ${e.apellido_paterno}`}>
            ...
            {calificacionesFiltradas.map((cal) => (
              <tr key={cal.id} className="bg-white border-b hover:bg-gray-50">
                <td className="px-6 py-4">
                  <StarRating rating={cal.calificacion} />
                </td>
                ...
                <td className="px-6 py-4">
                  {cal.cliente.nombre} {cal.cliente.apellido}
                </td>
