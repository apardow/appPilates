...
const PlusCircle: React.FC<{ size?: number; className?: string }> = ({
  size = 20,
  className,
}) => (
  <svg
...
const Edit: React.FC<{ size?: number; className?: string }> = ({
  size = 20,
  className,
}) => (
  <svg
...
const Trash2: React.FC<{ size?: number; className?: string }> = ({
  size = 20,
  className,
}) => (
  <svg
...
const ServiciosView: React.FC = () => {
    ...
    const fetchServicios = async () => {
      try {
        const response = await fetch(
          'https://api.espaciopilatescl.cl/api/servicios',
        );
        if (!response.ok) {
  ...
  const handleEliminar = async (id: number) => {
    ...
    if (window.confirm('¿Estás seguro de que deseas eliminar este servicio?')) {
      try {
        const response = await fetch(
          `https://api.espaciopilatescl.cl/api/servicios/${id}`,
          {
            method: 'DELETE',
          },
        );
