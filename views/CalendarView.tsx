...
const getSucursalColor = (sucursalId: number) =>
  sucursalColors[sucursalId % sucursalColors.length];
...
const CalendarView: React.FC = () => {
  ...
  const [claseSeleccionada, setClaseSeleccionada] =
    useState<ClaseDetallada | null>(null);
  ...
  const [hoveredClase, setHoveredClase] = useState<HoveredClaseInfo | null>(
    null,
  );
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      const [clasesRes, sucursalesRes, serviciosRes, clientesRes, configRes] =
        await Promise.all([
      ...
      const [clasesData, sucursalesData, serviciosData, clientesData] =
        await Promise.all([
          clasesRes.json(),
          sucursalesRes.json(),
          serviciosRes.json(),
  ...
  const handleClaseClick = async (claseId: number) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error('No se pudo cargar el detalle de la clase.');
  ...
  const handleCancelarClase = async (claseId: number) => {
    if (
      !window.confirm(
        '¿Estás seguro de que quieres cancelar esta clase? Se devolverán las clases a todas las alumnas inscritas.',
      )
    )
      return;
    setReservaStatus(null);
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.message || 'Error al cancelar la clase.');
      setReservaStatus({ message: data.message, type: 'success' });
  ...
  const clasesDeLaSemana = useMemo(() => {
    ...
    return todasLasClases.filter((clase) => {
      const sucursalMatch =
        ...
        clase.sucursal.id === parseInt(filtroSucursal);
      const servicioMatch =
        filtroServicio === 'todos' ||
        clase.servicio.id === parseInt(filtroServicio);
      return (
        sucursalMatch &&
        servicioMatch &&
        isWithinInterval(parseDateAsLocal(clase.fecha), { start, end })
      );
    });
  ...
  if (reservaBloqueada)
    razonesReserva.push(`Menos de ${policy.reservaMin} min de anticipación`);
  ...
  return (
    <div className="p-4 md:p-8">
          ...
          <div className="grid grid-cols-[auto_1fr] md:grid-cols-[60px_repeat(7,1fr)]">
            ...
            {Array.from({ length: 7 }).map((_, i) => {
              ...
              return (
                <div
                  key={dia.toString()}
                  className="text-center font-bold border-b border-r p-2 sticky top-0 bg-white z-10"
                >
              ...
              {Array.from({ length: END_HOUR - START_HOUR }).map((_, i) => (
                <div key={i} className="h-[60px] relative -top-2">
                  <span className="text-gray-500">
                    {format(new Date(0, 0, 0, START_HOUR + i), 'HH:mm')}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="row-start-2 col-start-2 grid grid-cols-1 md:grid-cols-7 col-span-1 md:col-span-7 relative">
              {Array.from({ length: END_HOUR - START_HOUR }).map((_, i) => (
                <div
                  key={i}
                  className="col-span-7 h-[60px] border-t border-gray-200"
                ></div>
              ))}
                    ...
                    {positionedClasesPorDia[diaFormateado]?.map((clase) => {
                      ...
                      return (
                        <div
                          key={clase.id}
                          onClick={() =>
                            !isCancelada && handleClaseClick(clase.id)
                          }
                          onMouseEnter={(e) =>
                            !isCancelada && handleMouseEnter(clase, e)
                          }
                          onMouseLeave={handleMouseLeave}
                          className={`absolute p-2 rounded-lg transition-shadow overflow-hidden flex flex-col ${
                            isCancelada
                              ? 'bg-red-50 border-red-200 opacity-60'
                              : `${colors.bg} border ${colors.border} cursor-pointer hover:shadow-lg`
                          }`}
                          style={{
                            top: `${clase.top}px`,
                            height: `${clase.height}px`,
                            left: `${clase.left}%`,
                            width: `${clase.width}%`,
                          }}
                        >
                          <p
                            className={`font-bold text-sm leading-tight ${isCancelada ? 'text-red-700 line-through' : colors.text}`}
                          >
                            {clase.servicio.nombre}
                          </p>
                          <p
                            className={`text-xs truncate ${isCancelada ? 'text-gray-500 line-through' : 'text-gray-600'}`}
                          >
                          ...
                          <div className="flex items-center justify-between text-xs mt-auto pt-1">
                            <span
                              className={`flex items-center gap-1 ${isCancelada ? 'text-gray-400' : 'text-blue-600'}`}
                            >
                              <CardAgendadosIcon />{' '}
                              <span className="font-semibold">
                                {clase.reservas_count}
                              </span>
                            </span>
                            <span
                              className={`flex items-center gap-1 ${isCancelada ? 'text-gray-400' : 'text-red-600'}`}
                            >
                              <CardEsperaIcon />{' '}
                              <span className="font-semibold">
                                {clase.lista_espera_count}
                              </span>
                            </span>
                          </div>
                        </div>
      ...
      {hoveredClase && (
        <div
          className="fixed bg-white rounded-lg shadow-xl p-4 z-50 w-64 border"
          style={{
            top: hoveredClase.position.top,
            left: hoveredClase.position.left,
          }}
        >
          <div className="flex justify-between items-center pb-2 mb-2 border-b">
            <h3 className="font-bold text-lg">
              {hoveredClase.clase.servicio.nombre}
            </h3>
            <span
              className={`${getSucursalColor(hoveredClase.clase.sucursal.id).tagBg} text-white text-xs font-bold px-3 py-1 rounded-full`}
            >
              Pilates
            </span>
          </div>
          <div className="text-sm space-y-2">
            <div className="flex items-center">
              <ClockIcon /> {hoveredClase.clase.hora_inicio.substring(0, 5)} -{' '}
              {hoveredClase.clase.hora_fin.substring(0, 5)}
            </div>
            <div className="flex items-center">
              <InstructorIcon /> {hoveredClase.clase.monitor.nombre}
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center mt-3 pt-3 border-t">
            <div>
              <p className="text-xl font-bold text-blue-600">
                {hoveredClase.clase.reservas_count}
              </p>
              <p className="text-xs text-gray-600">Agendados</p>
            </div>
            <div>
              <p className="text-xl font-bold text-green-600">
                {hoveredClase.clase.cupos_disponibles}
              </p>
              <p className="text-xs text-gray-600">Disponibles</p>
            </div>
            <div>
              <p className="text-xl font-bold text-red-600">
                {hoveredClase.clase.lista_espera_count}
              </p>
              ...
              <>
                <div className="flex justify-between items-center pb-3 mb-2">
                  <h2 className="text-xl font-bold text-gray-800">
                    {claseSeleccionada.servicio.nombre}
                  </h2>
                  <span
                    className={`${getSucursalColor(claseSeleccionada.sucursal.id).tagBg} text-white text-xs font-bold px-3 py-1 rounded-full`}
                  >
                    Pilates
                  </span>
                </div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Política: Reservar ≥ <strong>{policy.reservaMin}</strong> min
                  · Cancelar ≥ <strong>{policy.cancelMin}</strong> min
                </p>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                  ...
                  <div className="flex items-center">
                    <InstructorIcon /> {claseSeleccionada.monitor.nombre}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-center mb-4 border-t border-b py-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {claseSeleccionada.reservas.length}
                    </p>
                  ...
                  <div>
                    <DisponiblesIcon />
                    <p className="text-2xl font-bold">
                      {claseSeleccionada.cupos_disponibles}
                    </p>
                  ...
                  <div>
                    <EsperaIcon />
                    <p className="text-2xl font-bold">
                      {claseSeleccionada.lista_espera.length}
                    </p>
                    ...
                    <ul className="bg-gray-50 p-2 rounded h-32 overflow-y-auto text-sm border">
                      {claseSeleccionada.reservas.length > 0 ? (
                            ...
                            <button
                              ...
                              title={
                                cancelacionBloqueada
                                  ? `No se puede cancelar con menos de ${policy.cancelMin} minutos`
                                  : 'Cancelar reserva'
                              }
                            >
                              CANCELAR
                        ...
                        <li className="text-gray-500 p-1">
                          No hay alumnas agendadas.
                        </li>
                      )}
                    </ul>
                    {cancelacionBloqueada && (
                      <p className="text-xs text-gray-500 mt-2">
                        Las cancelaciones están bloqueadas a menos de{' '}
                        {policy.cancelMin} minutos del inicio.
                      </p>
                    )}
                  </div>
                ...
                <div className="mt-4 border-t pt-4">
                  ...
                  {esClaseLlena && !reservaBloqueada && clienteParaReservar && (
                    <p className="text-xs text-gray-600 mt-2">
                      Clase llena: al confirmar, la alumna quedará en{' '}
                ...
                <div className="mt-6 border-t pt-4 flex justify-end gap-3">
                  <button
                    onClick={() =>
                      navigate(`/clases/editar/${claseSeleccionada.id}`)
                    }
                ...
                {reservaStatus && (
                  <p
                    className={`mt-4 text-center text-sm ${reservaStatus.type === 'success' ? 'text-green-600' : 'text-red-600'}`}
                  >
