...
const SucursalesView: React.FC = () => {
    ...
    if (window.confirm('¿Estás seguro de que deseas eliminar esta sucursal?')) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || 'No se pudo eliminar la sucursal.',
          );
        }
  ...
  if (cargando)
    return <div className="p-8 text-center">Cargando sucursales...</div>;
  if (error)
    return (
      <div className="p-8">
        <div
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {sucursal.foto_url ? (
                        ...
                        <div className="w-16 h-10 bg-gray-200 rounded-md flex items-center justify-center text-xs text-gray-500">
                          Sin foto
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${sucursal.habilitado ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
