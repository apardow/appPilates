...
const EmpleadosView: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEmpleados = async () => {
            try {
                const response = await fetch('https://api.espaciopilatescl.cl/api/empleados');
                if (!response.ok) throw new Error('No se pudo obtener la lista de empleados.');
    ...
    const empleadosFiltrados = empleados.filter(e =>
        `${e.nombre} ${e.apellido_paterno}`.toLowerCase().includes(busqueda.toLowerCase())
    );

    if (cargando) return <div className="p-8 text-center">Cargando empleados...</div>;
                    ...
                    <table className="w-full text-sm text-left text-gray-500">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">Código Empleado</th>
                        ...
                        <tbody>
                            {empleadosFiltrados.map((empleado) => (
                                <tr key={empleado.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-medium text-gray-900">{empleado.nombre} {empleado.apellido_paterno}</td>
                                    <td className="px-6 py-4">{empleado.codigo_empleado}</td>
                                    <td className="px-6 py-4">{empleado.direccion_completa}</td>
                                    <td className="px-6 py-4 text-center">
                                        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${empleado.habilitado ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
